...
function OurTeamCard({ name, designation, image, description, linkedIn, twitter, more }) {
  ...
  return (
    <>
      ...
      {isModalOpen && (
        ...
        >
          <motion.div
            className="bg-white p-8 rounded-lg w-3/4 max-w-md"
            initial={{ scale: 0.8 }}
            animate={{ scale: 1 }}
            exit={{ scale: 0.8 }}
            transition={{ duration: 0.3 }}
          >
            <h2 className="text-xl font-bold text-gray-800 mb-4">More About {name}</h2>
            <p className="text-gray-600 text-justify">{more}</p>
